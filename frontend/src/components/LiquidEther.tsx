...
interface LiquidEtherProps {
    ...
    // Intentionally ignoring color0, color1, color2 props as they are handled via the colors array
    color0?: string;
    color1?: string;
    color2?: string;
}
    ...
    useEffect(() => {
        ...
        class MouseClass {
            ...
            init(container: HTMLElement) {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const defaultView = (this.docTarget && this.docTarget.defaultView) || ((typeof window !== 'undefined' ? window : null) as any);
...
`;
        ...
        class Advection extends ShaderPass {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            update(params?: any) { // Type compat fix
                if (params) {
        ...
        class ExternalForce extends ShaderPass {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            update(props?: any) { // Type compat fix
                if (props) {
        ...
        class Simulation {
            ...
            update() {
                ...
                this.output = this.fbos.vel_0;
            }
        }
        const SimulationInst = new Simulation({
        ...
        const OutputInst = new Output({
            vel: SimulationInst.fbos.vel_0
        });
        ...
        // Link internal mouse to autoDriver
        Mouse.autoIntensity = autoIntensity;
        Mouse.onInteract = () => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (autoDriver.manager as any).lastUserInteraction = performance.now();
        };
        ...
        class WebGLManager {
            ...
            init() {
                console.log('LiquidEther: WebGLManager init');
                if (Common.renderer) {
                    this.props.$wrapper.prepend(Common.renderer.domElement);
                }
                this.output = OutputInst;
            }

            resize() {
                Common.resize();
                SimulationInst.resize();
            }
            ...
            render() {
                ...
                SimulationInst.update();
                OutputInst.update({ vel: SimulationInst.output });
            }
            ...
            start() {
                if (this.running) return;
                console.log('LiquidEther: Starting loop');
                this.running = true;
                this._loop();
            }
        ...
        resizeObserverRef.current = ro; // storing ro to keep ref but types might mismatch if resizeRafRef is number
